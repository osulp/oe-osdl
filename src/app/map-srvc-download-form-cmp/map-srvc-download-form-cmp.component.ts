...
export class MapSrvcDownloadFormCmpComponent implements OnInit {
  ...
  checkDownload(record: any) {
    ...
    if (srvcUrl !== '' && record.links.length <= 1) {
      ...
      const gpService = L.esri.GP.service({
        url: 'http://lib-arcgis5.library.oregonstate.edu/arcgis/rest/services/_sandbox/URLToShapefile/GPServer/ZipExists'
      });
      ...
      this.hide();
      this.getDownload(record.links.length > 1 ? record.links[1] : '', record.title[0]);
    }
  }
  ...
  onFormSubmit(email: string) {
    const gpServiceDownload = L.esri.GP.service({
      url: 'http://lib-arcgis5.library.oregonstate.edu/arcgis/rest/services/_sandbox/URLToShapefile/GPServer/URLToShp'
    });
